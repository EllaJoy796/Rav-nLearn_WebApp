/* ═══════════════════════════════════════════════════════════════
   ...
   Requires: your existing CSS variables + Gemini API key config
   ═══════════════════════════════════════════════════════════════ */
...
(function injectStyles() {
    ...
    /* progress bar */
    #sbProgressBar {
        height: 3px; background: var(--accent-soft); flex-shrink: 0;
    }
    #sbProgressFill {
    ...
    #sbOrb.listening { border-color: #5dbf8a; animation: orbPulse 1.2s ease-in-out infinite; }
    @keyframes orbPulse {
        0%,100% { box-shadow: 0 0 0 0 rgba(124,92,191,.3); }
        50%      { box-shadow: 0 0 0 14px rgba(124,92,191,.0); }
    }
    #sbOrb.listening { animation: orbListenPulse 1.2s ease-in-out infinite; }
...
.sb-voice-item.selected {
    background: var(--accent-soft);
    border-color: var(--accent);
}
.sb-voice-name { font-size: 13px; font-weight: 600; color: var(--text-white); }
.sb-voice-lang { font-size: 11px; color: var(--text-muted); }
.sb-voice-badge {
...
(function injectHTML() {
    ...
    <div id="sbModal">
      ...
      <div id="sbHeader">
        ...
        <button id="sbCloseBtn" onclick="SB.close()">✕</button>
      </div>
      ...
      <!-- VOICE PICKER (shown before quiz starts) -->
      <div id="sbVoicePicker" style="display:none; flex-direction:column; gap:12px; padding:24px 28px;">
        <div style="font-size:13px; font-weight:700; color:var(--text-muted); letter-spacing:.5px; text-transform:uppercase;">Choose a Voice</div>
        <div id="sbVoiceList" style="display:flex; flex-direction:column; gap:8px; max-height:260px; overflow-y:auto; scrollbar-width:thin; scrollbar-color:var(--border) transparent;"></div>
        <button id="sbStartBtn" onclick="window.SB.startWithVoice()" style="height:42px; background:var(--accent); border:none; border-radius:12px; color:#fff; font-size:14px; font-weight:700; font-family:inherit; cursor:pointer; margin-top:4px; transition:background .2s;">
          ▶ Start Session
        </button>
      </div>
      ...
      <div id="sbStage">
        <div id="sbOrb">
          <svg id="sbOrbIcon" viewBox="0 0 24 24">
        ...
        <!-- fallback text input (shown if no speech recognition) -->
        <div id="sbFallbackRow">
          <input id="sbFallbackInput" type="text" placeholder="Type your answer…" />
          <button id="sbFallbackSubmit" onclick="SB.submitTextAnswer()">Submit</button>
        </div>
      ...
      <div id="sbResults">
        ...
        <div class="sb-result-btns">
          <button class="sb-btn-retry" onclick="SB.retry()">↺ Try Again</button>
          <button class="sb-btn-done" onclick="SB.close()">Done</button>
        </div>
      </div>
      ...
      <div id="sbFooter">
        ...
        <div id="sbFooterBtns">
          <button class="sb-btn" id="sbEndBtn" onclick="SB.close()">
            <svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/></svg>
            End session
          </button>
          <button class="sb-btn" id="sbSkipBtn" onclick="SB.skip()">
            Skip
            <svg viewBox="0 0 24 24"><polyline points="9 18 15 12 9 6"/></svg>
          </button>
          <button class="sb-btn" id="sbMicBtn" onclick="SB.toggleMic()">
...
const SB = (() => {
    ...
    function setOrb(mode) { // 'idle' | 'speaking' | 'listening' | 'thinking'
        orb().className = '';
...
Be friendly and encouraging. No bullet points. Speak naturally as if talking to a friend.`;
    ...
    function showResults() {
        ...
        $('sbProgressFill').style.width = '100%';

        $('sbResScore').textContent = `${score}/${questions.length}`;
        const pct = questions.length > 0 ? Math.round((score / questions.length) * 100) : 0;
        $('sbResTitle').textContent = pct >= 80
            ? '🎉 Excellent work!'
            : pct >= 50
            ? '👍 Good effort!'
            : '💪 Keep practicing!';
        $('sbResSub').textContent =
        ...
        const msg = pct >= 80
            ...
            : `You got ${score} out of ${questions.length}. Don't worry — reviewing is how we learn. Try again and you'll do better!`;
        speak(msg);
    }
    ...
    return {
        open(label, qs) {
            ...
            if (!speechSupported) {
                console.warn('[StudyBuddy] SpeechRecognition not supported — fallback input mode.');
            }

            // start after a short settle
            const greeting = `Hi! I'm your Study Buddy! I'll be quizzing you on ${label}. Just speak your answers out loud — or type them if you prefer. Say "ready" or click Listen when you're good to go!`;

setTimeout(() => {
    speak(greeting, () => {
        // After greeting, show "Say ready to begin" then wait for mic
        stateLabel().textContent = 'SAY "READY" TO BEGIN';
        setTab('user');
        waitingAns = false; // not a real question yet
        ...
        if (SR) {
            ...
            r.onerror  = ()  => showQuestion(); // fallback: just start
            r.onend    = ()  => {};
            try { r.start(); } catch(e) { showQuestion(); }
        } else {
            // no speech support — add a "Start" button
            const btn = document.createElement('button');
            btn.textContent = '▶ Start Quiz';
            btn.className = 'sb-btn';
            btn.style.cssText = 'background:var(--accent);border:none;color:#fff;margin-top:8px;';
            btn.onclick = () => { btn.remove(); showQuestion(); };
            document.getElementById('sbStage').appendChild(btn);
        }
    });
}, 400);
        },
